...
| `ade-install.sh` | Clones/pulls repos, installs GSD + kitchens, regenerates the Cursor workspace file |
| `<scaffold-dir>.code-workspace` | Multi-root Cursor workspace listing every portfolio repo that cloned successfully. Regenerated by `ade-install.sh` from `ade-repos.txt` (preserves any non-portfolio roots and non-managed settings you add); opened automatically on first scaffold only |
